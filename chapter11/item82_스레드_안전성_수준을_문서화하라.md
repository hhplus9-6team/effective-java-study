...
- 예를 들어 Collections.synchronizedMap의 API 문서에는 다음과 같이 써있다.
  ...
  Set<K> s = m.keySet(); // 동기화 블록 밖에 있어도 된다. ... synchronzed(m) { // s가 아닌 m을 사용해 동기화해야 한다. for(K key : s) key.f(); }
    ```
...
### 질문
- synchronized 키워드가 붙어 있으면 그 메서드는 스레드 안전하다고 볼 수 없다고 하는 이유를 설명해주세요
- 스레드 안전성 수준에 따라 문서화하는 방법을 설명해주세요