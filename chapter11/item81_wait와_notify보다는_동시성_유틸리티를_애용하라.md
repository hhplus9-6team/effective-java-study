...
동시성 장치(Synchronizers)는

* 여러 스레드가 **언제 시작하고**
* **언제까지 기다리며**
* **어떤 조건이 만족되면 진행할지**

를 `wait / notify` 없이 표현하기 위한 고수준 도구다.
...
이들은 **데이터를 공유하기 위한 컬렉션**이 아니라,
**스레드의 흐름을 맞추기 위한 도구**다.

---

## 6. 왜 동시성 장치가 필요한가

`wait / notify`로 스레드 협업을 구현하면 다음 문제가 발생한다.
* 조건 변수 관리 필요
* while 루프 필수
* notify vs notifyAll 선택 문제
* 신호 유실(missed signal) 가능성
* 코드 의도가 드러나지 않음
...
```
* `ready` : 모든 작업 스레드가 준비되었는지 확인
* `start` : 모든 작업을 동시에 시작시키기 위한 신호
* `done` : 모든 작업이 끝났는지 확인
...
반면,
* 일반적인 웹 요청 처리 로직에서는 거의 사용되지 않는다.
...
CountDownLatch보다 **프로덕션 코드에서 등장할 가능성이 높다**.

대표 사례:

* 외부 API 호출 수 제한
* 레거시 시스템 보호
* 동시에 접근 가능한 리소스 개수 제한

특징:
* “몇 개까지 허용할 것인가”를 표현할 때 적합
* 단순 락(`Lock`)으로는 표현하기 어려운 문제를 해결
...
* 정해진 수의 스레드가 모두 도착할 때까지 대기
* 모두 도착하면 **전부 동시에 다음 단계로 이동**
* CountDownLatch와 다르게 재사용 가능

```java
CyclicBarrier barrier = new CyclicBarrier(3);

barrier.await(); // 3개 모이면 전부 진행
```
